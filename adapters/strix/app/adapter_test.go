...
// The full commander loop over bufconn gRPC: propose → verdict → execute
// only what was accepted → TaskResult mapped back.
func TestSubmitThenExecuteApprovedTask(t *testing.T) {
	...
	out, err := call(t, d.submitTaskPlan, `{
		...
		"tasks": [
			{"task_key": "scan-web", "capability": "detect.scan.web", "risk_class": "R2",
			 "targets": ["https://acme.com"], "params": {"instruction": "Focus on the login flow."}},
	...
	// The plan reached the PlannerService tagged with the Strix commander id.
	if len(fake.Plans()) != 1 || fake.Plans()[0].GetSubmittedBy() != CommanderID {
	...
	if first["mock"] != true {
		t.Errorf("mock mode must be visible in the mapped result: %v", first)
	}
	findings := first["findings"].([]any)
	if len(findings) != 1 {
		t.Fatalf("findings = %v, want 1 canned finding", findings)
	}
	f0 := findings[0].(map[string]any)
	if f0["mock"] != true || !strings.HasPrefix(f0["id"].(string), "MOCK-") {
		t.Errorf("canned finding must be clearly marked: %v", f0)
	}
...
func TestSubmitValidation(t *testing.T) {
	...
	cases := []struct {
		...
		{"no mission", `{"tasks":[{"task_key":"a","capability":"recon.port_scan","risk_class":"R1","targets":["x"]}]}`, "mission_id"},
		{"no tasks", `{"mission_id":"msn_x","tasks":[]}`, "at least one task"},
		{"dup keys", `{"mission_id":"msn_x","tasks":[
			{"task_key":"a","capability":"recon.port_scan","risk_class":"R1","targets":["x"]},
			{"task_key":"a","capability":"recon.port_scan","risk_class":"R1","targets":["x"]}]}`, "duplicate task_key"},
		{"bad risk", `{"mission_id":"msn_x","tasks":[{"task_key":"a","capability":"recon.port_scan","risk_class":"R9","targets":["x"]}]}`, "risk_class"},
		{"bad dep", `{"mission_id":"msn_x","tasks":[{"task_key":"a","capability":"recon.port_scan","risk_class":"R1","targets":["x"],"depends_on":["ghost"]}]}`, "unknown task_key"},
	}
...
func TestReadTools(t *testing.T) {
	...
	if fake.ScopeChanges()[0].GetRequestedBy() != CommanderID {
		t.Errorf("scope change requested_by = %v, want Strix commander id", fake.ScopeChanges()[0].GetRequestedBy())
	}
}
