// Package app is the Strix commander adapter core: it fronts the local Strix
// installation as a platform commander (doc 01 §4.1, §7.1).
//
// The adapter is a PLANNER, NOT AN AUTHORIZER. It submits TaskPlans to the
// Orchestrator's PlannerService and may only translate a task into a Strix
// scan after the Orchestrator's PlanVerdict accepted that task. Accepted
// tasks live in the Ledger; execute_approved_task consults nothing else.
// Authorization for real dispatch stays with the Orchestrator dispatch PEP
// and the gatekeeper PDP (Ruling B) — this adapter never sees, mints, or
// verifies a Scope Token.
package app
