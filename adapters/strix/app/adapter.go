...
// CommanderID is the Strix commander identity submitted as
// TaskPlan.submitted_by / RequestScopeChangeRequest.requested_by.
// platform/v1/types.proto defines COMMANDER_STRIX=3, and the Orchestrator's
// commanderName/commanderMaxRisk tables map it to the "strix" commander
// (R0–R2 proposal band, doc 01 §4.1).
const CommanderID = platformv1.Commander_COMMANDER_STRIX

// Deps are the adapter's external dependencies, injected so tests can wire
// the adapter to a bufconn PlannerService and the mock Strix client.
type Deps struct {
	// Planner is the client side of aegisbastion.platform.v1.PlannerService.
	Planner platformv1.PlannerServiceClient
	// Strix runs scans (mock by default).
	Strix strixcli.Client
...
// Orchestrator-accepted task into Strix scans and maps the findings back to
// a TaskResult.
func RegisterTools(s *mcp.Server, d *Deps) {
	...
	s.RegisterTool(mcp.Tool{
		...
		Description: "Translate one Orchestrator-ACCEPTED task (by plan_id + task_key from a prior " +
			"submit_task_plan verdict) into Strix scans (one non-interactive strix run per target) and " +
			"return the TaskResult with PoC-validated findings. Refuses any task the Orchestrator did not " +
			"accept: the adapter is a planner, not an authorizer.",
		InputSchema: objectSchema(map[string]any{
...
func (d *Deps) submitTaskPlan(_ *mcp.CallContext, raw json.RawMessage) (any, error) {
	...
	plan, err := taskspec.BuildPlan(CommanderID, "", &args)
	if err != nil {
		return nil, err
	}
...
func (d *Deps) executeApprovedTask(_ *mcp.CallContext, raw json.RawMessage) (any, error) {
	...
	// THE GATE: only Orchestrator-accepted tasks may be translated into Strix
	// scans. This is the adapter's enforcement that it is a planner, not an
	// authorizer.
	...
	started := d.Now()
	results := make([]*strixcli.ScanResult, len(calls))
	callErrs := make([]error, len(calls))
	for i, call := range calls {
		results[i], callErrs[i] = d.Strix.RunScan(ctx, call)
		if ctx.Err() != nil {
			// Stop fanning out once the task deadline is hit; remaining scans
...
var submitPlanSchema = objectSchema(map[string]any{
	...
	"idempotency_key": stringProp("Optional commander-scoped idempotency key, e.g. strix:msn_01J8ZK:plan:7"),
	...
	"tasks": map[string]any{
		...
		"items": objectSchema(map[string]any{
			...
			"params":      map[string]any{"type": "object", "description": "Capability-specific parameters (instruction, scan_mode)"},
