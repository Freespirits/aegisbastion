...
import { LiveSink, type AdapterContext } from "./dispatch/adapters.js";
...
import { busSinks, startIngestConsumer, type IngestConsumer } from "./buswire.js";

export async function startHerald(): Promise<{ stop: () => Promise<void> }> {
  ...
  try {
    ...
    console.error(`herald: JWKS unavailable at boot (${(err as Error).message}) — authz-required alerts will be HELD`);
  }
  ...
  let consumer: IngestConsumer | null = null;
  let sinks = noopSinks;
  if (cfg.busEnabled) {
  ...
  const pipeline = new Pipeline(
    pipelineOptionsFromConfig(cfg, { store, enforcer, assetCache, sinks, metrics }),
  );

  // --- dispatch (C6) -----------------------------------------------------------
  const resolveSecret = (ref: string): string | undefined =>
    process.env[`HERALD_SECRET_${ref.toUpperCase().replace(/[^A-Z0-9]/g, "_")}`];
  ...
  const dispatchLoop = new DispatchLoop(dispatcher, cfg.dispatchScanMs, (err) =>
    console.error(`herald: dispatch loop error: ${(err as Error).message}`),
  );
  const escalationLoop = new EscalationLoop(store, pipeline, cfg.escalationScanMs, (err) =>
    console.error(`herald: escalation loop error: ${(err as Error).message}`),
  );
  const authzRetryTimer = setInterval(() => {
    pipeline.runDueAuthzRetries(new Date()).catch((err) =>
      console.error(`herald: authz retry loop error: ${(err as Error).message}`),
    );
  }, cfg.authzRetryMs);
  ...
  await new Promise<void>((resolveListen) => server.listen(Number(portStr), host, resolveListen));
...
if (isMain) {
  startHerald().catch((err) => {
    console.error(`herald: fatal: ${(err as Error).stack ?? err}`);
    process.exit(1);
  });
}
