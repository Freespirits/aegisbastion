...
import { parseCidr, ipv4InCidr, ipv6InCidr, parseIpv4, parseIpv6, type Cidr } from "@aegisbastion/agent-sdk";
...
const blockedV4: Cidr[] = BLOCKED_V4.map((s) => parseCidr(s)!).filter((c) => c.family === 4);
const blockedV6: Cidr[] = BLOCKED_V6.map((s) => parseCidr(s)!).filter((c) => c.family === 6);
...
export type SsrfVerdict =
  | { allow: true; addresses: { address: string; family: number }[] }
  | { allow: false; reason: string };

export interface SsrfOptions {
  /** Admin-flagged internal destination (org egress policy entry internal: true). */
 ...
 */
export async function guardDestination(rawUrl: string, opts: SsrfOptions = {}): Promise<SsrfVerdict> {
  let url: URL;
  try {
    url = new URL(rawUrl);
  } catch {
  ...
  try {
    ...
    return { allow: false, reason: `DNS resolution failed for ${hostname}: ${(err as Error).message}` };
  }
...
export function pinnedLookup(addresses: { address: string; family: number }[]) {
  return (
    _hostname: string,
    options: { all?: boolean },
