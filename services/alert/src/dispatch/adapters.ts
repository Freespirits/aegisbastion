...
import type { AlertEvent, Channel, Incident, Severity } from "../types.js";
...
export interface AdapterContext {
  /** Default Slack incoming webhook (compose: ALERT_SLACK_WEBHOOK_URL) for "#channel" destinations. */
  slackDefaultWebhookUrl: string;
  /** Default Teams incoming webhook for "#channel" destinations. */
  teamsDefaultWebhookUrl: string;
  /** Default Splunk HEC token (ALERT_SPLUNK_HEC_TOKEN); per-endpoint overrides via secret resolver. */
  splunkHecToken: string;
  /** Per-endpoint secret resolution (egress entry secret_ref → secret value; §13.5). */
  resolveSecret: (ref: string) => string | undefined;
  /** Fallback webhook signing secret (ALERT_WEBHOOK_SIGNING_SECRET). */
  defaultWebhookSecret: string;
  timeoutMs: number;
  /** DNS resolver injection (tests). */
  resolveAll?: (hostname: string) => Promise<{ address: string; family: number }[]>;
  /** Syslog dial injection (tests): returns a writable socket-like. */
  syslogDial?: (destination: string) => Promise<Socket>;
}
...
function destinationUrl(destination: string, fallback: string): string {
...
async function guardedPost(
  ...
  try {
    const res = await postJson(new URL(rawUrl), body, {
      ...
      lookup: pinnedLookup(verdict.addresses) as never,
    });
...
export function renderSlackPayload(req: SendRequest): Record<string, unknown> {
...
export async function sendSlack(req: SendRequest, ctx: AdapterContext): Promise<SendResult> {
  const url = destinationUrl(req.delivery.destination, ctx.slackDefaultWebhookUrl);
  if (!url) return { ok: false, error: "no Slack webhook URL configured for destination" };
  const payload = renderSlackPayload(req);
  const mention = (req.delivery as { mention?: string }).mention;
  if (mention) (payload as { text: string }).text = `${mention} ${payload.text as string}`;
...
export function renderTeamsPayload(req: SendRequest): Record<string, unknown> {
  const { incident, text } = req;
  return {
    type: "message",
    attachments: [
...
export async function sendTeams(req: SendRequest, ctx: AdapterContext): Promise<SendResult> {
  const url = destinationUrl(req.delivery.destination, ctx.teamsDefaultWebhookUrl);
  if (!url) return { ok: false, error: "no Teams webhook URL configured for destination" };
  return guardedPost(url, JSON.stringify(renderTeamsPayload(req)), {}, ctx, req.egressEntry?.internal === true);
}

// ---------------------------------------------------------------------------
// Generic signed webhook (§6/§13.5): CloudEvents envelope body, HMAC
// X-AegisBastion-Signature over ts.body, 5 s timeout, SSRF guard, no redirects.
// ---------------------------------------------------------------------------
...
export interface WebhookEnvelope {
  ...
  data: { incident: Incident; alerts: AlertEvent[]; text: string };
}

export function renderWebhookEnvelope(req: SendRequest, now: Date): WebhookEnvelope {
  return {
    specversion: "1.0",
    id: `dlvmsg_${ulid()}`,
    source: "//aegisbastion/alert",
...
export async function sendWebhook(req: SendRequest, ctx: AdapterContext, now: Date = new Date()): Promise<SendResult> {
  ...
  const result = await guardedPost(
    req.delivery.destination,
    body,
    { "x-aegisbastion-signature": signature, "x-aegisbastion-delivery": req.delivery.deliveryId },
    ctx,
    req.egressEntry?.internal === true,
  );
  if (result.ok) result.payloadSnapshot = { signature, envelope: safeJson(body) };
  return result;
}
...
export function renderSplunkEvent(req: SendRequest, alert: AlertEvent): Record<string, unknown> {
  return {
...
export async function sendSplunkBatch(reqs: SendRequest[], ctx: AdapterContext): Promise<SendResult> {
  const first = reqs[0]!;
  ...
  const token =
    (first.egressEntry?.secret_ref ? ctx.resolveSecret(first.egressEntry.secret_ref) : undefined) ?? ctx.splunkHecToken;
  const events = reqs.flatMap((req) => req.alerts.map((a) => renderSplunkEvent(req, a)));
  const body = events
    .map((e) => JSON.stringify(index ? { ...e, index } : e))
    .join("\n");
  const result = await guardedPost(
    `${url.replace(/\/$/, "")}/services/collector`,
    body,
    token ? { authorization: `Splunk ${token}` } : {},
    ctx,
    first.egressEntry?.internal === true,
  );
  if (result.ok) result.payloadSnapshot = { events: events.length };
  return result;
}
...
const SYSLOG_SEVERITY: Record<Severity, number> = { critical: 2, high: 3, medium: 4, low: 5, info: 6 };
const CEF_SEVERITY: Record<Severity, number> = { critical: 10, high: 8, medium: 5, low: 2, info: 1 };

function cefEscape(s: string): string {
  return s.replace(/\\/g, "\\\\").replace(/([|=])/g, "\\$1").replace(/[\r\n]+/g, " ");
}

export function renderSyslogLine(req: SendRequest, alert: AlertEvent, now: Date = new Date()): string {
...
function parseSyslogDestination(destination: string): { host: string; port: number; useTls: boolean } {
  const m = /^(?:(tcp|tls):\/\/)?([^:/]+):(\d+)$/.exec(destination);
  if (!m) throw new Error(`invalid syslog destination ${destination} (want [tcp|tls://]host:port)`);
  return { host: m[2]!, port: Number(m[3]), useTls: m[1] === "tls" };
}

async function defaultDial(destination: string): Promise<Socket> {
  const { host, port, useTls } = parseSyslogDestination(destination);
  return new Promise((resolveSocket, rejectSocket) => {
    const onError = (err: Error) => rejectSocket(err);
    if (useTls) {
...
export async function sendSyslog(req: SendRequest, ctx: AdapterContext): Promise<SendResult> {
  const started = Date.now();
  ...
  try {
    ...
    return { ok: false, latencyMs: Date.now() - started, error: `syslog connect failed: ${(err as Error).message}` };
  }
  try {
    ...
    return { ok: false, latencyMs: Date.now() - started, error: `syslog write failed: ${(err as Error).message}` };
  } finally {
    sock.destroy();
  }
}
...
export class LiveSink implements DeliverySink {
  readonly mode = "live";
  ...
  async send(req: SendRequest): Promise<SendResult> {
    try {
      ...
      return { ok: false, error: `adapter error: ${(err as Error).message}` };
    }
  }
...
export const ADAPTER_CHANNELS: readonly Channel[] = ["slack", "teams", "splunk-hec", "syslog", "webhook"];
