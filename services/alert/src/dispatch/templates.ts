...
import type { AlertEvent, Incident } from "../types.js";
...
const TEMPLATES: Record<string, string> = {
  ...
  still_firing:
    "still firing (×{{incident.alertCount}}): *{{sev}}* {{incident.title}} — incident {{incident.incidentId}}",
  escalation:
    "ESCALATION: *{{sev}}* {{incident.title}} is unacknowledged — incident {{incident.incidentId}}{{#if mention}} {{mention}}{{/if}}",
  digest:
    "alert digest for {{incident.orgId}}: {{incident.alertCount}} alert(s) in the current storm window, latest: {{incident.title}} ({{incident.incidentId}})",
  notify: "{{#each alerts}}{{this.title}}\n{{this.description}}{{/each}}{{#if note}}\n{{note}}{{/if}}",
  plain: "{{incident.title}} — {{#each alerts}}{{this.description}}{{/each}}",
};

const compiled = new Map<string, Handlebars.TemplateDelegate<TemplateContext>>();

function templateFor(name: string): Handlebars.TemplateDelegate<TemplateContext> {
  ...
  if (!t) {
    ...
    t = Handlebars.compile<TemplateContext>(TEMPLATES[key]!, { noEscape: true, strict: true });
    compiled.set(key, t);
  }
  return t;
}
...
export function renderText(template: string, ctx: TemplateContext): RenderedText {
  const full = { ...ctx, sev: SEV_BADGE[ctx.incident.severity] ?? `[${ctx.incident.severity}]` };
  try {
    return { text: templateFor(template)(full as never) };
  } catch (err) {
    // §12 template render error → plain-text fallback, incident still delivered.
    try {
      return {
        text: templateFor("plain")(full as never),
        renderError: (err as Error).message,
      };
    } catch {
      return { text: `${ctx.incident.title} (incident ${ctx.incident.incidentId})`, renderError: (err as Error).message };
    }
  }
}
