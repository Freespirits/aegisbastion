...
interface MemoryIncident extends Incident {
  alertIds: string[];
}

interface MemoryDelivery extends Delivery {
  payload?: unknown;
  attempts: unknown[];
}

export function dedupVerdictFor(count: number, renotifyEvery: number): "new" | "duplicate" | "renotify" {
  if (count <= 1) return "new";
  if (renotifyEvery > 0 && count % renotifyEvery === 0) return "renotify";
  return "duplicate";
}

export class MemoryStore implements Store {
  alerts = new Map<string, StoredAlert>();
  incidents = new Map<string, MemoryIncident>();
  routing = new Map<string, RoutingPolicy>(); // policyId
  escalationPolicies = new Map<string, EscalationPolicy>();
  egress = new Map<string, EgressEntry[]>(); // orgId
  deliveries = new Map<string, MemoryDelivery>();
  dedup = new Map<string, { orgId: string; alertId: string; count: number; firstSeen: number; expiresAt: number }>();
  audit: (AuditRecord & { auditId: string; ts: Date; forwardedAt?: Date })[] = [];
  ackNonces = new Set<string>();

  async insertAlertIfNew(row: StoredAlert): Promise<"inserted" | "duplicate"> {
    if (this.alerts.has(row.eventId)) return "duplicate";
  ...
  async listAlerts(filter: AlertFilter): Promise<StoredAlert[]> {
    ...
    if (filter.incidentId) rows = rows.filter((r) => r.incidentId === filter.incidentId);
    if (filter.severityGte) {
      const min = severityRank(filter.severityGte as never);
  ...
  async setAlertAuthz(
    ...
    if (claims !== undefined) a.authzClaims = structuredClone(claims);
    if (retryAt !== undefined) (a as { authzRetryAt?: Date | null }).authzRetryAt = retryAt;
  }

  async setAlertIncident(eventId: string, incidentId: string): Promise<void> {
    const a = this.alerts.get(eventId);
    if (a) a.incidentId = incidentId;
  }
  ...
  async dueAuthzRetries(now: Date): Promise<StoredAlert[]> {
    return [...this.alerts.values()]
      .filter((a) => {
        if (a.authzStatus !== "held") return false;
        const retryAt = (a as { authzRetryAt?: Date | null }).authzRetryAt;
  ...
  async attachAlertToIncident(incidentId: string, event: AlertEvent, now: Date): Promise<void> {
    ...
    if (event.requires_ack) inc.requiresAck = true;
    if (severityRank(event.severity) > severityRank(inc.severity)) inc.severity = event.severity;
  }
  ...
  async ackIncident(
    ...
    if (this.ackNonces.has(nonce)) return "nonce_used";
    const inc = this.incidents.get(incidentId);
    if (!inc) return "notfound";
    if (inc.state === "acknowledged" || inc.state === "resolved") return "already";
  ...
  async setIncidentEscalation(
    ...
    if (exhausted !== undefined) inc.escalationExhausted = exhausted;
  }

  async dueEscalations(now: Date): Promise<Incident[]> {
    return [...this.incidents.values()]
      .filter((inc) => {
        if (!inc.requiresAck) return false;
        if (inc.state !== "open" && inc.state !== "escalated") return false;
  ...
  async putEgressPolicy(orgId: string, entries: EgressEntry[], _updatedBy: string): Promise<void> {
  ...
  async recordDeliveryAttempt(deliveryId: string, result: DeliveryAttemptResult): Promise<void> {
    ...
    if (result.nextAttemptAt) d.nextAttemptAt = result.nextAttemptAt;
    if (result.status === "sent") d.sentAt = new Date();
    if (result.error !== undefined) d.error = result.error;
    if (result.payloadSnapshot !== undefined) d.payload = result.payloadSnapshot;
  }
  ...
  async listDeliveries(filter: DeliveryFilter): Promise<Delivery[]> {
    ...
    if (filter.orgId) rows = rows.filter((r) => r.orgId === filter.orgId);
    if (filter.incidentId) rows = rows.filter((r) => r.incidentId === filter.incidentId);
    if (filter.alertId) rows = rows.filter((r) => r.alertIds.includes(filter.alertId!));
  ...
  async deliveryStatusCounts(orgId?: string): Promise<Record<DeliveryStatus, number>> {
    ...
    for (const d of this.deliveries.values()) {
      if (orgId && d.orgId !== orgId) continue;
      counts[d.status] += 1;
    }
    return counts;
  }
  ...
  async markAuditForwarded(auditId: string, at: Date): Promise<void> {
    ...
    if (row) row.forwardedAt = at;
  }

  async close(): Promise<void> {}
}

/** Test helper: a Channel-shaped passthrough for building MemoryDelivery rows. */
export type { Channel };
