...
import type { AlertEvent, SourceModule } from "../types.js";
...
export function isAuthzUnavailable(err: unknown): boolean {
...
export function requiresAuthorization(event: AlertEvent): boolean {
  if (event.source_module === "ddos-engine" || event.source_module === "ai-redteam") return true;
  return (
    (event.category === "vuln" || event.category === "exposure") && event.confidence === "confirmed"
  );
}
 ...
 */
const MODULE_CAPABILITY_PREFIXES: Record<SourceModule, string[]> = {
...
export function capabilitiesCoverModule(capabilities: string[], module: SourceModule): boolean {
...
export type AuthzVerdict =
  ...
  | { outcome: "verified"; claims: ScopeTokenClaims }
...
export interface AuthzEnforcerOptions {
  ...
  /** Verified-manifest memoization (keyed by manifest_sha256) — avoids a MinIO round-trip per alert. */
  manifestCache?: Map<string, { at: number; manifest: VerifiedManifest }>;
  manifestCacheTtlMs?: number;
}

export class AuthzEnforcer {
  private readonly cache: Map<string, { at: number; manifest: VerifiedManifest }>;
  private readonly cacheTtl: number;

  constructor(private readonly opts: AuthzEnforcerOptions) {
   ...
   */
  async verify(event: AlertEvent, compactToken: string | undefined, now: Date): Promise<AuthzVerdict> {
    ...
    const guardedGetKey: typeof this.opts.jwksCache.getKey = async (header, token) => {
      try {
    ...
    try {
      ...
      if (code === "JWKS_UNAVAILABLE" && !isAuthzUnavailable(jwksError)) {
        return {
          ...
          reason: `no gatekeeper signing key matches the token kid: ${(err as Error).message}`,
        };
      }
      if (isAuthzUnavailable(err) || isAuthzUnavailable(jwksError)) {
        return { outcome: "unavailable", reason: `verification unavailable: ${(err as Error).message}` };
      }
      return {
        outcome: "rejected",
        code: `AUTHZ_${code}`,
    ...
    try {
      ...
      if (isAuthzUnavailable(err)) {
        return { outcome: "unavailable", reason: `manifest verification unavailable: ${(err as Error).message}` };
      }
      const code = (err as { code?: string }).code ?? "MANIFEST_INVALID";
      return { outcome: "rejected", code: `AUTHZ_${code}`, reason: (err as Error).message };
    }
  ...
  private async verifiedManifest(claims: ScopeTokenClaims): Promise<VerifiedManifest> {
    const key = claims.targets.manifest_sha256;
    const hit = this.cache.get(key);
    const nowMs = Date.now();
    if (hit && nowMs - hit.at < this.cacheTtl) return hit.manifest;
