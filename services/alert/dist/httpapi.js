...
import { CHANNELS, SEVERITIES } from "./types.js";
function send(res, status, body, headers = {}) {
    const payload = typeof body === "string" ? body : JSON.stringify(body);
    res.writeHead(status, { "content-type": "application/json", ...headers });
    res.end(payload);
}
function isAdmin(deps, actor) {
...
function severityParam(v) {
    return SEVERITIES.includes(v ?? "") ? v : undefined;
}
// ---------------------------------------------------------------------------
// Route handlers
// ---------------------------------------------------------------------------
async function handleIngestAlert(deps, ctx) {
    const body = ctx.body;
    if (!body || typeof body !== "object")
        return badRequest(ctx.res, "JSON body required");
...
async function handleNotify(deps, ctx) {
    const body = ctx.body;
    if (!body || typeof body !== "object")
        return badRequest(ctx.res, "JSON body required");
    const payload = body.payload;
    if (typeof body.org_id !== "string" || body.org_id === "")
        return badRequest(ctx.res, "org_id is required");
    if (!payload || typeof payload.title !== "string" || payload.title === "") {
        return badRequest(ctx.res, "payload.title is required");
    }
    const channelOverride = Array.isArray(body.channel_override) ? body.channel_override : undefined;
    ...
    const result = await deps.pipeline.notify({
        ...
        ...(severityParam(typeof body.severity_floor === "string" ? body.severity_floor : null)
            ? { severity_floor: severityParam(body.severity_floor) }
            : {}),
        ...(severityParam(typeof body.severity === "string" ? body.severity : null)
            ? { severity: severityParam(body.severity) }
            : {}),
        ...
        ...(body.asset && typeof body.asset === "object" ? { asset: body.asset } : {}),
    });
    return send(ctx.res, result.status === "rejected" ? 403 : 202, result);
}
async function handleAckPost(deps, ctx) {
    const body = ctx.body;
    if (!body || typeof body !== "object")
        return badRequest(ctx.res, "JSON body required");
    ...
    if (typeof body.token === "string") {
        ...
        if (!decoded)
            return send(ctx.res, 403, { error: "invalid or expired ack callback token" });
    ...
    if (!incidentId && typeof body.alert_id === "string") {
        ...
        if (!incidentId)
            return send(ctx.res, 404, { error: "alert has no incident (or unknown alert_id)" });
    }
    if (!incidentId)
        return badRequest(ctx.res, "incident_id (or alert_id, or token) is required");
...
async function handleAckCallback(deps, ctx) {
    const token = ctx.url.searchParams.get("token") ?? "";
    const decoded = verifyAckToken(deps.config.ackSigningSecret, token);
    if (!decoded)
        return send(ctx.res, 403, { status: "invalid_or_expired_token" });
...
function parseRoutingPolicy(body, actor, existing) {
    ...
    for (const t of targets) {
        if (!CHANNELS.includes(t.channel))
            return `invalid channel: ${String(t.channel)}`;
        if (typeof t.destination !== "string" || t.destination === "")
            return "every target needs a destination";
    }
    const match = (body.match ?? existing?.match ?? {});
    return {
...
function parseEscalationPolicy(body, existing) {
    const orgId = typeof body.org_id === "string" ? body.org_id : existing?.orgId;
    if (!orgId)
        return "org_id is required";
    ...
    return {
        ...
        orgId,
        steps: steps,
        repeatLastStepEverySeconds: typeof body.repeat_last_step_every_seconds === "number"
        ...
        stopOn: Array.isArray(body.stop_on) ? body.stop_on : (existing?.stopOn ?? ["ack", "resolved"]),
    };
}
function parseEgressEntries(body) {
    const entries = body?.entries ?? body;
    if (!Array.isArray(entries))
        return "entries array required";
    for (const e of entries) {
        if (!CHANNELS.includes(e.channel))
            return `invalid channel: ${String(e.channel)}`;
        if (typeof e.pattern !== "string" || e.pattern === "")
            return "every entry needs a pattern";
    }
    return entries;
}
// ---------------------------------------------------------------------------
...
export function createHttpServer(deps) {
    ...
    const routes = [
        ...
        },
        {
            ...
            handler: async ({ res, params }) => {
                ...
                if (!inc)
                    return send(res, 404, { error: "incident not found" });
        ...
        },
        {
            ...
            handler: async ({ res, body, actor }) => {
                const parsed = parseRoutingPolicy(body, actor);
        ...
        },
        {
            ...
            handler: async ({ res, body, actor, params }) => {
                ...
                const parsed = parseRoutingPolicy({ ...body, policy_id: params.id }, actor, existing ?? undefined);
                if (typeof parsed === "string")
                    return badRequest(res, parsed);
                await store.putRoutingPolicy(parsed);
                await store.appendAudit({
        ...
        },
        {
            ...
            admin: true,
            handler: async ({ res, body, actor }) => {
                const parsed = parseEscalationPolicy(body);
                if (typeof parsed === "string")
                    return badRequest(res, parsed);
        ...
        },
        {
            ...
            handler: async ({ res, body, actor, params }) => {
                ...
                const existing = typeof orgId === "string" ? await store.escalationPolicy(orgId, params.id) : null;
                const parsed = parseEscalationPolicy({ ...body, policy_id: params.id }, existing ?? undefined);
                if (typeof parsed === "string")
                    return badRequest(res, parsed);
                await store.putEscalationPolicy(parsed);
                await store.appendAudit({
    ...
    return createServer(async (req, res) => {
        try {
            ...
            console.error(`herald: http error: ${err.stack ?? err}`);
            if (!res.headersSent)
