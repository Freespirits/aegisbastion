...
import { applyEgressPolicy, bootstrapPolicies, defaultEscalationPolicy, routeIncident, urgencyForSeverity, } from "./routing.js";
import { initialEscalationState } from "./escalate.js";
import { newDeliveryId } from "./ids.js";
import { severityRank } from "./types.js";
export const noopSinks = {
...
export class Pipeline {
    ...
    // -------------------------------------------------------------------------
    async audit(action, orgId, entityIds, decisionDetail, actor = { kind: "service", id: this.actor }) {
        const record = {
            orgId,
            actor,
            action,
    ...
    async ingest(event, ctx) {
        ...
        const claims = verdict.outcome === "verified"
            ? { jti: verdict.claims.jti, task_id: verdict.claims.task_id, roe_id: verdict.claims.roe_id, risk_class: verdict.claims.risk_class, capabilities: verdict.claims.capabilities }
            : undefined;
        const inserted = await this.insertAlertRow(enriched, ctx, effectiveSeverity, "open", authzStatus, claims);
        if (!inserted)
            return { status: "duplicate" }; // §3.2 step 2: duplicates on event_id dropped at ingest
    ...
    async insertAlertRow(event, ctx, effectiveSeverity, state, authzStatus, authzClaims) {
        const row = {
    ...
    async processStored(event, effectiveSeverity, now) {
    ...
    // -------------------------------------------------------------------------
    async runDueAuthzRetries(now) {
        const due = await this.opts.store.dueAuthzRetries(now);
        let processed = 0;
        for (const row of due) {
            const heldToken = row.authzClaims?.held_token;
    ...
    async quarantine(row, code, reason) {
    ...
    // -------------------------------------------------------------------------
    async routeAndQueue(incident, event, now, stillFiring) {
        const store = this.opts.store;
    ...
    async createDeliveries(incident, targets, opts) {
        let created = 0;
    ...
    async fireStep(incident, step, _policy) {
        const egress = await this.egressFor(incident.orgId);
    ...
    async resolve(incidentId, actor) {
    ...
    async notify(order) {
        ...
        const inserted = await this.insertAlertRow(enriched, { receivedAt: now }, effectiveSeverity, "open", verdict.outcome === "verified" ? "verified" : "none");
        if (!inserted)
            return { status: "duplicate" };
        ...
        for (const raw of order.channel_override) {
            ...
            if (idx <= 0)
                continue;
            overrides.push({ channel: raw.slice(0, idx), destination: raw.slice(idx + 1) });
        }
    ...
    async escalationPolicyFor(orgId, policyId) {
...
export function pipelineOptionsFromConfig(cfg, base) {
    let egressSeed;
    if (cfg.egressSeedJson) {
        egressSeed = JSON.parse(cfg.egressSeedJson);
    }
    return {
        ...base,
        bootstrapChannels: {
