...
function mapAlertRow(r) {
    return {
        ...
        dedupVerdict: r.dedup_verdict,
        dedupDegraded: r.dedup_degraded,
        authzStatus: r.authz_status,
        incidentId: r.incident_id ?? undefined,
        receivedAt: new Date(r.received_at),
        authzClaims: r.authz ?? undefined,
        event: r.raw,
    };
}
function mapIncidentRow(r) {
    return {
        ...
        escalationExhausted: r.escalation_exhausted,
    };
}
function mapDeliveryRow(r) {
    return {
        ...
        escalationStep: r.escalation_step ?? undefined,
...
export class PostgresStore {
    ...
    async insertAlertIfNew(row) {
        const e = row.event;
        const res = await this.pool.query(`INSERT INTO alert.alerts (
    ...
    async setAlertFingerprint(eventId, fingerprint) {
    ...
    async listAlerts(filter) {
        ...
        if (filter.severityGte) {
            ...
            where.push(`array_position(ARRAY['info','low','medium','high','critical'], effective_severity) >= array_position(ARRAY['info','low','medium','high','critical'], $${args.length})`);
        }
        if (filter.cursor) {
    ...
    async setAlertAuthz(eventId, status, claims, retryAt) {
        await this.pool.query(`UPDATE alert.alerts SET authz_status = $2,
         authz = COALESCE($3, authz),
         authz_retry_at = $4
       WHERE event_id = $1`, [eventId, status, claims ? JSON.stringify(claims) : null, retryAt ?? null]);
    }
    ...
    async dueAuthzRetries(now) {
        const res = await this.pool.query(`SELECT * FROM alert.alerts WHERE authz_status = 'held' AND authz_retry_at IS NOT NULL AND authz_retry_at <= $1
       ORDER BY authz_retry_at LIMIT 100`, [now]);
        return res.rows.map(mapAlertRow);
    }
    async dedupTouch(fingerprint, orgId, alertId, windowSeconds, renotifyEvery, _now) {
        ...
        const res = await this.pool.query(`INSERT INTO alert.dedup_state (fingerprint, org_id, alert_id, occurrence_count, first_seen_at, last_seen_at, expires_at)
       ...
       RETURNING occurrence_count, alert_id`, [fingerprint, orgId, alertId, windowSeconds]);
        if (res.rows[0]) {
            const count = res.rows[0].occurrence_count;
            return {
                verdict: dedupVerdictFor(count, renotifyEvery),
                count,
                firstAlertId: res.rows[0].alert_id,
                degraded: false,
            };
        }
        ...
        const retry = await this.pool.query(`INSERT INTO alert.dedup_state (fingerprint, org_id, alert_id, occurrence_count, first_seen_at, last_seen_at, expires_at)
       VALUES ($1, $2, $3, 1, now(), now(), LEAST(now() + make_interval(secs => $4), now() + interval '7 days'))
       ...
       RETURNING occurrence_count, alert_id`, [fingerprint, orgId, alertId, windowSeconds]);
        ...
        const winner = await this.pool.query(`SELECT alert_id, occurrence_count FROM alert.dedup_state WHERE fingerprint = $1`, [fingerprint]);
        const count = winner.rows[0]?.occurrence_count ?? 2;
        return {
            verdict: dedupVerdictFor(Math.max(count, 2), renotifyEvery),
            count,
            firstAlertId: winner.rows[0]?.alert_id ?? alertId,
            degraded: false,
        };
    }
    async findOpenIncident(orgId, correlationKey) {
        const res = await this.pool.query(`SELECT * FROM alert.incidents
       WHERE org_id = $1 AND correlation_key = $2 AND state IN ('open','acknowledged','escalated')
       ORDER BY first_seen_at DESC LIMIT 1`, [orgId, correlationKey]);
        return res.rows[0] ? mapIncidentRow(res.rows[0]) : null;
    }
    async insertIncident(incident) {
        await this.pool.query(`INSERT INTO alert.incidents (
    ...
    async listIncidents(filter) {
        ...
        if (filter.severityGte) {
            args.push(filter.severityGte);
            where.push(`array_position(ARRAY['info','low','medium','high','critical'], severity) >= array_position(ARRAY['info','low','medium','high','critical'], $${args.length})`);
        }
        args.push(filter.limit ?? 100);
        const res = await this.pool.query(`SELECT * FROM alert.incidents ${where.length ? `WHERE ${where.join(" AND ")}` : ""}
       ORDER BY last_seen_at DESC, incident_id DESC LIMIT $${args.length}`, args);
    ...
    async attachAlertToIncident(incidentId, event, now) {
        ...
        try {
            ...
            await client.query(`INSERT INTO alert.incident_alerts (incident_id, event_id) VALUES ($1, $2) ON CONFLICT DO NOTHING`, [incidentId, event.event_id]);
            await client.query(`UPDATE alert.incidents SET
           ...
           updated_at = now()
         WHERE incident_id = $1`, [incidentId, now, event.severity, event.requires_ack ?? false]);
    ...
    async incidentAlerts(incidentId) {
        const res = await this.pool.query(`SELECT event_id FROM alert.incident_alerts WHERE incident_id = $1 ORDER BY attached_at`, [incidentId]);
    ...
    async ackIncident(incidentId, by, note, nonce, now) {
        ...
        try {
            await client.query("BEGIN");
            try {
                await client.query(`INSERT INTO alert.acks (ack_id, org_id, incident_id, actor, note, nonce, created_at)
           SELECT $1, org_id, $2, $3, $4, $5, $6 FROM alert.incidents WHERE incident_id = $2`, [`ack_${Date.now().toString(36)}${Math.random().toString(36).slice(2, 10)}`, incidentId, by, note, nonce, now]);
            }
            catch (err) {
                if (err.code === "23505") {
                    await client.query("ROLLBACK");
            ...
            const res = await client.query(`UPDATE alert.incidents SET state = 'acknowledged', ack = $2, updated_at = now()
         WHERE incident_id = $1 AND state NOT IN ('acknowledged','resolved') RETURNING incident_id`, [incidentId, JSON.stringify({ by, at: now.toISOString(), note })]);
            if (res.rowCount === 0) {
            ...
            await client.query(`UPDATE alert.alerts SET state = 'acknowledged' WHERE incident_id = $1 AND state = 'open'`, [incidentId]);
            await client.query("COMMIT");
            return "acked";
        }
        catch (err) {
    ...
    async resolveIncident(incidentId, _now) {
        await this.pool.query(`UPDATE alert.incidents SET state = 'resolved', updated_at = now() WHERE incident_id = $1`, [incidentId]);
        await this.pool.query(`UPDATE alert.alerts SET state = 'resolved' WHERE incident_id = $1 AND state IN ('open','acknowledged')`, [incidentId]);
    }
    async setIncidentEscalation(incidentId, escalation, state, exhausted) {
        await this.pool.query(`UPDATE alert.incidents SET escalation = $2,
         state = COALESCE($3, state),
         escalation_exhausted = COALESCE($4, escalation_exhausted),
         updated_at = now()
       WHERE incident_id = $1`, [incidentId, JSON.stringify(escalation), state ?? null, exhausted ?? null]);
    }
    async dueEscalations(now) {
       ...
       ORDER BY (escalation->>'next_fire_at')::timestamptz LIMIT 100`, [now]);
        return res.rows.map(mapIncidentRow);
    }
    async routingPolicies(orgId) {
        ...
        const res = orgId === ""
            ? await this.pool.query(`SELECT * FROM alert.routing_policies WHERE enabled ORDER BY org_id, priority, created_at`)
            : await this.pool.query(`SELECT * FROM alert.routing_policies WHERE org_id = $1 AND enabled ORDER BY priority, created_at`, [orgId]);
        return res.rows.map((r) => ({
            ...
            match: r.match,
            targets: r.targets,
            escalationPolicyId: r.escalation_policy_id ?? undefined,
            suppressIfAcknowledgedWithin: r.suppress_if_acknowledged_within ?? undefined,
    ...
    async putRoutingPolicy(policy) {
        await this.pool.query(`INSERT INTO alert.routing_policies
    ...
    async escalationPolicy(orgId, policyId) {
        const res = await this.pool.query(`SELECT * FROM alert.escalation_policies WHERE org_id = $1 AND policy_id = $2`, [orgId, policyId]);
        const r = res.rows[0];
        if (!r)
            return null;
        return {
            ...
            orgId: r.org_id,
            steps: r.steps,
            repeatLastStepEverySeconds: r.repeat_last_step_every_seconds,
            maxRepeats: r.max_repeats,
            stopOn: r.stop_on,
        };
    }
    async putEscalationPolicy(policy) {
        await this.pool.query(`INSERT INTO alert.escalation_policies
    ...
    async egressPolicy(orgId) {
        ...
        return res.rows[0] ? res.rows[0].entries : null;
    }
    async putEgressPolicy(orgId, entries, updatedBy) {
        await this.pool.query(`INSERT INTO alert.egress_policies (org_id, entries, updated_by)
       VALUES ($1, $2, $3)
       ON CONFLICT (org_id) DO UPDATE SET
         entries = EXCLUDED.entries, updated_by = EXCLUDED.updated_by,
         version = alert.egress_policies.version + 1, updated_at = now()`, [orgId, JSON.stringify(entries), updatedBy]);
    }
    async insertDelivery(delivery, payloadSnapshot) {
        await this.pool.query(`INSERT INTO alert.deliveries
    ...
    async dueDeliveries(now, limit) {
        const res = await this.pool.query(`SELECT * FROM alert.deliveries
    ...
    async recordDeliveryAttempt(deliveryId, result) {
        await this.pool.query(`UPDATE alert.deliveries SET
    ...
    async listDeliveries(filter) {
        ...
        args.push(filter.limit ?? 100);
        const res = await this.pool.query(`SELECT * FROM alert.deliveries ${where.length ? `WHERE ${where.join(" AND ")}` : ""}
       ORDER BY created_at DESC, delivery_id DESC LIMIT $${args.length}`, args);
        return res.rows.map(mapDeliveryRow);
    }
    async deliveryStatusCounts(orgId) {
        const res = await this.pool.query(`SELECT status, count(*)::int AS n FROM alert.deliveries ${orgId ? "WHERE org_id = $1" : ""} GROUP BY status`, orgId ? [orgId] : []);
    ...
    async appendAudit(record) {
        ...
        await this.pool.query(`INSERT INTO alert.audit_log (audit_id, org_id, actor, action, entity_ids, decision_detail, request_hash)
